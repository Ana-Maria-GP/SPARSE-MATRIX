...
__global__ void sparseMatrixVectorMultiplyGPU(const SparseMatrix* Md, const float* v, float* result);
...
int main(int argc, char* argv[]) {
    ...
    if (mode == 1) {
        ...
        // Calcular el tamaño de la cuadrícula basado en el tamaño del problema y el tamaño del bloque
        int gridSize = (n + blockSize - 1) / blockSize;

        // Definir dim3 para el bloque y la cuadrícula
        dim3 blockDim(blockSize);
        dim3 gridDim(gridSize);

        cudaEventRecord(start);
        sparseMatrixVectorMultiplyGPU<<<gridDim, blockDim>>>(&Md, v, d_result);
...
__global__ void sparseMatrixVectorMultiplyGPU(const SparseMatrix* Md, const float* v, float* result) {
    ...
    if (tid < Md->nnz) {
        atomicAdd(&result[Md->elements[tid].row], Md->elements[tid].value * v[Md->elements[tid].col]);
    }
}
